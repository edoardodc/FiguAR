...
class Service: NSObject {
    ...
    static func addChildNode(_ node: SCNNode, toNode: SCNNode, inView: ARSCNView, cameraRelativePosition: SCNVector3) {
        ...
        var traslationMatrix = matrix_identity_float4x4
        traslationMatrix.columns.3.x = cameraRelativePosition.x
        traslationMatrix.columns.3.y = cameraRelativePosition.y
        traslationMatrix.columns.3.z = cameraRelativePosition.z
        let modifiedMatrix = simd_mul(transform, traslationMatrix)
